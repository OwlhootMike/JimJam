...
take_turn = function() {
	...
	if(_dist_x <= abs(CELL_SIZE * 7) && _dist_y <= abs(CELL_SIZE * 7)){
        ...
        if (obj_jimothy.x > x) {
			...
			image_speed = 1;
			sprite_index = poison_rat_bite;
			instance_create_depth(x, y, -100, obj_ratChomp);
	        obj_jimothy.hp -= 1; 
			obj_jimothy.hpShow = true;
			
			if(obj_jimothy.poison == false){
				obj_jimothy.poison = true;
				obj_jimothy.poisonTick = 0;	
				instance_create_depth(obj_jimothy.x, obj_jimothy.y, -100, obj_poisoned);
			}
